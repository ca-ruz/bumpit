...
def test_unreserve_on_failure(node_factory):
    ...
    channel_funding_output = next(
        (output for output in outputs if not output['reserved'] and output["amount_msat"] == 200000000),
        None
    )
    print(f"~~~~~~~~~~~~~~~~channel funding output: {channel_funding_output}")
    assert channel_funding_output is not None

    utxos=[
        f"{channel_funding_output['txid']}:{channel_funding_output['output']}"
        ]
    print(f"~~~~~~~~~~~~~~~~utxos: {utxos}")

    # Fund channel, keep transaction unconfirmed
    funding = l1.rpc.fundchannel(
        l2.info['id'], 100000, 
        feerate="3000perkb", 
        utxos=utxos
        )  # 100,000 satoshis
