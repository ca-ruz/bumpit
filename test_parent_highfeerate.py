...
def test_bumpchannelopen_high_parent_fee(node_factory):
    ...
    # Setup and fund channel with a high initial fee rate
    ...
    # Fund channel with a high fee rate (e.g., 50 sat/vB)
    funding = l1.rpc.fundchannel(l2.info['id'], FUNDAMOUNT, feerate="50000perkb")  # 50 sat/vB
    funding_txid = funding['txid']

    # Find change output
    outputs = l1.rpc.listfunds()['outputs']
    change_output = next((o for o in outputs if o['txid'] == funding_txid and not o['reserved']), None)
    assert change_output is not None, "No unreserved change output found"

    # Calculate parent details
    parent_details = calculate_parent_tx_details(bitcoind, funding_txid)
    print(f"Parent fee rate: {parent_details['feerate']} sat/vB")
    assert parent_details['feerate'] > 40, "Parent fee rate should be high"

    # Bump to a lower target (e.g., 10 sat/vB)
    target_feerate = 3
    try:
        ...
        # Verify: Since parent fee rate is high, total feerate should still meet or exceed target
        total_feerate = result['total_feerate']
        assert total_feerate >= target_feerate, "Total feerate should meet or exceed target"
        assert result['third_child_fee_sat"'] >= 0, "Child fee should be non-negative"
        assert abs(result['parent_feerate'] - parent_details['feerate']) < 0.1, "Parent feerate mismatch"
    except RpcError as e:
        print(f"RPC Error: {str(e)}")
        # Allow graceful failure if plugin can't handle high parent fee yet
        assert "Error creating CPFP transaction" in str(e), "Unexpected error message"